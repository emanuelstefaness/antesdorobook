...
import { Camera, CheckCircle2, ExternalLink, Share2, TriangleAlert } from "lucide-react";
  ...
  return (
    <section className="max-w-[1020px] rounded-card border-2 border-cyan/30 bg-cyan/5 p-5 sm:p-8">
      {showBuild ? <>
        ...
        <div className="mt-6 rounded-card-sm bg-navy p-5 text-white"><h3 className="font-display text-[18px] font-extrabold">5. Código copiável e compartilhável</h3><p className="mt-2 text-[12.5px] leading-relaxed text-white/70">No MakeCode, troque de Blocos para JavaScript, substitua o conteúdo por este código e volte para Blocos. Confira o mapa visual antes de transferir.</p>{guide.codeNote ? <p className="mt-3 rounded bg-amber/15 p-3 text-[12px] leading-relaxed text-amber">{guide.codeNote}</p> : null}<pre className="mt-4 overflow-x-auto rounded-card-sm bg-black/35 p-4 text-[11.5px] leading-relaxed text-cyan"><code>{guide.code}</code></pre><div className="mt-4 flex flex-wrap gap-3"><CopiarCodigo code={guide.code}/><BaixarCodigo code={guide.code} filename={guide.id}/><a href="https://makecode.microbit.org/#editor" target="_blank" rel="noreferrer" className="no-print inline-flex min-h-[40px] items-center gap-2 rounded-pill border border-white/25 px-4 text-[12px] font-bold text-white">Abrir MakeCode <ExternalLink size={14}/></a></div><div className="mt-5 border-t border-white/15 pt-4"><div className="flex gap-3"><Share2 className="mt-0.5 shrink-0 text-cyan"/><p className="text-[12.5px] leading-relaxed text-white/75"><strong className="text-white">Criar o link da turma:</strong> clique em Compartilhar, dê um título, publique o projeto e teste o endereço em uma janela anônima. O portal fornece o código correto, mas não inventa um link público que ainda não foi publicado no MakeCode.</p></div></div></div>
