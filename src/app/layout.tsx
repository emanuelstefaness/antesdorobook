...
import { PortaoDeAcesso } from "@/components/auth/PortaoDeAcesso";
...
const SCRIPT_ACESSO = `try {
  var v = localStorage.getItem('${KEYS.acesso}');
  document.documentElement.dataset.acesso = v === ${JSON.stringify(JSON.stringify(HASH_DA_SENHA))} ? 'liberado' : 'trancado';
} catch (e) {
  document.documentElement.dataset.acesso = 'trancado';
}`;
  ...
  return (
    ...
    >
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: SCRIPT_ACESSO }} />
      </head>
      <body>
        <PortaoDeAcesso>
          <SkipLink />
