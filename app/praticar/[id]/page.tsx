...
export default async function AtividadePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <header className="border-b border-navy/8 bg-cream-hi">
        <div className="mx-auto max-w-[1400px] px-5 py-7 md:px-8">
          ...
          <div className="relative mt-5 aspect-[3.35/1] overflow-hidden rounded-card shadow-card">
            <Image unoptimized fill priority src={atividade.id === "algoritmo-do-sanduiche" ? "/assets/antes-do-robo/atividade-algoritmo-sanduiche.png" : `/imagens/praticar/${atividade.id}.jpg`} alt={`Atividade ${atividade.title} em sala`} className="object-cover object-center" />
            <span className="absolute bottom-3 left-3 rounded-pill bg-white/92 px-4 py-2 text-[11px] font-bold uppercase tracking-wide text-navy shadow">Atividade de sala</span>
          </div>
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
            {[`${atividade.duration} min`, anos, "Turma inteira", "Preparação rápida", `Nível ${atividade.level}`, atividade.materials.includes("computador") ? "Com computador" : "Sem computador"].map((x)=><span key={x} className="rounded-xl border border-navy/8 bg-white px-3 py-3 text-center text-[11px] font-semibold shadow-sm">{x}</span>)}
          </div>
        </div>
      </header>
